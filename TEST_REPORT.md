...
- The second test account `optimizeddiet@gmail.com` can open the draft listing and sees **Install**. With explicit approval for the five permissions, attempted installation. Google opened `script.google.com/accounts?authuser=1` with a continuation to the script's authorization page, but the page displayed **Page Not Found** before consent. The account's Apps Script home loads and has no projects. Retrying from the listing after reload produced the same result. Independent installation and formula execution remain unverified; cause is not established.
...
- Created a separate blank workbook under `optimizeddiet@gmail.com` for an independent tester, but the draft add-on could not yet be installed there. The Marketplace overlay search did not index the draft; the direct listing opened, but Install led to Apps Script's Page Not Found before consent. A separate browser profile with only that account signed in is the next diagnostic, without sharing the script project.
